...
export default function DisposalWasteConfig() {
  ...
  const [selectedDisposalMethod, setSelectedDisposalMethod] =
    useState<DisposalMethodWithWasteType | null>(null);
  const [editingWaste, setEditingWaste] = useState<WasteType | null>(null);
  const [editingMethod, setEditingMethod] = useState<DisposalMethodWithWasteType | null>(null);
  ...
  const handleUpdateWasteType = async (id: string, name: string, emissionFactor: number) => {
    try {
      setLoading(true);
  ...
  return (
    <div className="grid gap-4">
      ...
      >
        <ProFormText name="name" label="Disposal Method Name" rules={[{ required: true }]} />
      </ModalForm>

      {/* Add/Edit Waste Type */}
      <ModalForm
        title={
          editingWaste ? 'Edit Waste Type' : `Add Waste Type for ${selectedDisposalMethod?.name}`
        }
        ...
        onFinish={async (values) => {
          if (editingWaste) {
            ...
            await handleUpdateWasteType(editingWaste.id, values.name, values.emissionFactor);
      ...
      >
        <ProFormText name="name" label="Waste Type Name" rules={[{ required: true }]} />
        <ProFormDigit name="emissionFactor" label="Emission Factor" rules={[{ required: true }]} />
      </ModalForm>
    </div>
  );
}
