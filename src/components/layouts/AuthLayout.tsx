  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-25 to-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute top-20 left-10 w-8 h-8 bg-green-200 rounded-full opacity-20 animate-bounce"
          style={{ animationDelay: '0s', animationDuration: '3s' }}
        ></div>
        <div
          className="absolute top-40 right-20 w-6 h-6 bg-emerald-300 rounded-full opacity-30 animate-bounce"
          style={{ animationDelay: '1s', animationDuration: '4s' }}
        ></div>
        <div
          className="absolute top-96 left-20 w-10 h-10 bg-green-100 rounded-full opacity-25 animate-bounce"
          style={{ animationDelay: '2s', animationDuration: '5s' }}
        ></div>
        <div
          className="absolute top-80 right-10 w-7 h-7 bg-lime-200 rounded-full opacity-20 animate-bounce"
          style={{ animationDelay: '3s', animationDuration: '3.5s' }}
        ></div>
        <div
          className="absolute bottom-32 left-1/4 w-9 h-9 bg-emerald-200 rounded-full opacity-15 animate-bounce"
          style={{ animationDelay: '4s', animationDuration: '4.5s' }}
        ></div>
        <div
          className="absolute bottom-60 right-1/3 w-5 h-5 bg-green-300 rounded-full opacity-25 animate-bounce"
          style={{ animationDelay: '5s', animationDuration: '3.8s' }}
        ></div>

        {/* Decorative Geometric Shapes */}
        <div
          className="absolute top-1/4 left-5 w-16 h-16 border-2 border-green-200 rotate-45 opacity-10 animate-spin"
          style={{ animationDuration: '20s' }}
        ></div>
        <div className="absolute top-2/3 right-8 w-12 h-12 border-2 border-emerald-300 rotate-12 opacity-15 animate-pulse"></div>
        <div
          className="absolute bottom-1/4 left-1/3 w-14 h-14 border-2 border-lime-200 rotate-90 opacity-12 animate-spin"
        ...
        <div
          className="absolute top-2/3 left-1/4 opacity-8 animate-pulse"
        ...
        <div
          className="absolute bottom-1/3 right-1/4 opacity-12 animate-pulse"
      ...
      <header
        className={`relative ${headerHeights[headerHeight]} bg-gradient-to-r from-green-700 via-green-600 to-emerald-700 flex flex-col justify-center items-center space-y-4 shadow-2xl overflow-hidden`}
      >
      ...
      <main
        className={`container mx-auto px-4 py-8 ${headerHeight === 'small' ? '-mt-12' : headerHeight === 'medium' ? '-mt-15' : '-mt-20'} relative z-20`}
      >
        {/* Form Container with Enhanced Styling */}
        <div className={`relative ${containerSizes[containerSize]} mx-auto`}>
          {/* Background Glow Effect */}
          <div
            className={`absolute inset-0 bg-gradient-to-r from-green-100 to-emerald-100 ${containerSize === 'large' ? 'rounded-3xl' : 'rounded-2xl'} blur-xl opacity-30 scale-105`}
          ></div>

          {/* Form */}
          <div
            className={`relative bg-white/${containerSize === 'large' ? '95' : '90'} backdrop-blur-sm ${containerSize === 'large' ? 'rounded-3xl' : 'rounded-2xl'} shadow-2xl ${containerSize === 'large' ? 'p-2 border border-green-100' : 'p-1'}`}
          >
            {children}
          </div>
        </div>
